...
class SeeCommands extends PluginCommand{
	...
	public function __construct($name, Main $plugin) {
        ...
        $this->setDescription("Check a player's xp");
        $this->setUsage("/seeexp <player>");
		$this->setAliases(["xp"]);
        $this->setPermission("core.command.xp");
		$this->plugin = $plugin;
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool{
		if (count($args) < 1) {
            $sender->sendMessage(TextFormat::GOLD . "Usage:" . TextFormat::GREEN . " /seeexp <player>");
            return false;
        }
		if (!empty($args[0])) {
            ...
            if ($target == true) {
				$exp = $target->getCurrentTotalXp();
				$sender->sendMessage("§a" . $target->getDisplayName() . "§r§a's EXP: §b" . number_format($exp));
				return true;
            }
		}
	}
